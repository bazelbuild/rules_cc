...
// Returns the resolved path, or "" on NOT_FOUND. Retries once with the
// reported size on BUF_TOO_SMALL; fails the test on any other error.
...
TEST(RunfilesCTest, RlocationOverridesSourceRepo) {
  ...
  // No mapping from the main repository, so the directory fallback applies.
  EXPECT_EQ(Rloc(rf, "my_module/file"), argv0 + ".runfiles/my_module/file");
  // From src_repo, my_module rewrites to canonical_v1.
...
// The prefix lookup must not write to the caller's path.
...
TEST(RunfilesCTest, IsAbsolute) {
  ...
  EXPECT_EQ(rf_is_absolute("//host/share"), 0);
