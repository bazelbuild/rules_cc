...
load("//cc/private/toolchain:windows_cc_configure.bzl", "configure_windows_toolchain")
load("//cc/private/toolchain:osx_cc_configure.bzl", "configure_osx_toolchain")
load("//cc/private/toolchain:unix_cc_configure.bzl", "configure_unix_toolchain")
load(
...
def _generate_cpp_only_build_file(repository_ctx, cpu_value, paths):
    repository_ctx.template(
        "BUILD",
        paths["//cc/private/toolchain:BUILD.toolchains.tpl"],
        {"%{name}": cpu_value},
    )
...
def cc_autoconf_toolchains_impl(repository_ctx):
    ...
    if not should_detect_cpp_toolchain:
        ...
        if not should_use_xcode:
            ...
            # And also so we don't have to keep this code in sync with @bazel_tools//tools/cpp:osx_cc_configure.bzl.
...
def cc_autoconf_impl(repository_ctx, overriden_tools = dict()):
    ...
    if "BAZEL_DO_NOT_DETECT_CPP_TOOLCHAIN" in env and env["BAZEL_DO_NOT_DETECT_CPP_TOOLCHAIN"] == "1":
        repository_ctx.symlink(paths["//cc/private/toolchain:cc_toolchain_config.bzl"], "cc_toolchain_config.bzl")
        repository_ctx.symlink(Label("//cc/private/toolchain:BUILD.empty"), "BUILD")
        ...
        repository_ctx.symlink(paths["//cc/private/toolchain:cc_toolchain_config.bzl"], "cc_toolchain_config.bzl")
        repository_ctx.symlink(paths["//cc/private/toolchain:BUILD.static.freebsd"], "BUILD")
