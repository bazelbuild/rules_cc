...
namespace cov = bazel_coverage;

int main() {
  cov::SetVerbose(cov::HasEnv("VERBOSE_COVERAGE"));

  std::string coverage_dir = cov::RequireEnv("COVERAGE_DIR");
  ...
  std::string tool = cov::GetEnvOr("BAZEL_CC_COVERAGE_TOOL", "GCOV");
  if (cov::UsesLlvm(coverage_dir)) {
    tool = cov::GetEnvOr("GENERATE_LLVM_LCOV", "") == "1" ? "LLVM_LCOV"
                                                          : "PROFDATA";
  }

  if (tool == "GCOV") {
    return cov::GcovCoverage(coverage_dir,
                             cov::JoinPath(coverage_dir, "_cc_coverage.gcov"));
  }
  if (tool == "PROFDATA") {
    return cov::LlvmCoverageProfdata(
        coverage_dir, cov::JoinPath(coverage_dir, "_cc_coverage.profdata"));
  }
  if (tool == "LLVM_LCOV") {
    return cov::LlvmCoverageLcov(
        coverage_dir, cov::JoinPath(coverage_dir, "_cc_coverage.dat"));
  }
