...
def _compute_tool_paths(toolchain_config_info, crosstool_top_path, output_directory_prefix):
    ...
    for tool in toolchain_config_info.tool_paths:
        ...
        if path_str.startswith(output_directory_prefix):
            # The path of a file generated by the build, e.g. a compiler wrapper built in the
            # exec configuration, is already relative to the execution root.
            tool_paths_collector[tool.name] = path_str
        else:
...
def get_cc_toolchain_provider(ctx, attributes):
    ...
    tool_paths = _compute_tool_paths(
        toolchain_config_info,
        tools_directory,
        # The first segment of the bin directory path, e.g. "bazel-out/", is shared by the output
        # directories of all configurations.
        bin_dir_path[:bin_dir_path.find("/") + 1],
    )
