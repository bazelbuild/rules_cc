...
load("@rules_cc//cc:windows_cc_configure.bzl", "configure_windows_toolchain")
load("@rules_cc//cc:osx_cc_configure.bzl", "configure_osx_toolchain")
load("@rules_cc//cc:unix_cc_configure.bzl", "configure_unix_toolchain")
load(
    "@rules_cc//cc:lib_cc_configure.bzl",
    "get_cpu_value",
    "resolve_labels",
)
...
def _generate_cpp_only_build_file(repository_ctx, cpu_value, paths):
    repository_ctx.template(
        "BUILD",
        paths["@rules_cc//cc:BUILD.toolchains.tpl"],
        {"%{name}": cpu_value},
    )
...
def cc_autoconf_toolchains_impl(repository_ctx):
    ...
    paths = resolve_labels(repository_ctx, [
        "@rules_cc//cc:BUILD.toolchains.tpl",
    ...
    if not should_detect_cpp_toolchain:
        ...
        if not should_use_xcode:
            ...
            # And also so we don't have to keep this code in sync with @rules_cc//cc:osx_cc_configure.bzl.
...
def cc_autoconf_impl(repository_ctx, overriden_tools = dict()):
    ...
    """
    paths = resolve_labels(repository_ctx, [
        "@rules_cc//cc:BUILD.static.freebsd",
    ...
    if "BAZEL_DO_NOT_DETECT_CPP_TOOLCHAIN" in env and env["BAZEL_DO_NOT_DETECT_CPP_TOOLCHAIN"] == "1":
        repository_ctx.symlink(paths["@rules_cc//cc:cc_toolchain_config.bzl"], "cc_toolchain_config.bzl")
        repository_ctx.symlink(Label("@rules_cc//cc:BUILD.empty"), "BUILD")
        ...
        repository_ctx.symlink(paths["@rules_cc//cc:cc_toolchain_config.bzl"], "cc_toolchain_config.bzl")
        repository_ctx.symlink(paths["@rules_cc//cc:BUILD.static.freebsd"], "BUILD")
