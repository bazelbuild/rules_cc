...
// rf_allocator::malloc backed by libc.
//
// @param ud Ignored.
...
// rf_allocator::realloc backed by libc.
//
// @param ud Ignored.
...
// rf_allocator::free backed by libc.
//
// @param ud Ignored.
// @param p Allocation to release, or NULL.
...
// @param a Allocator to dispatch to.
// @param n Bytes to allocate.
...
// @param p Existing allocation, or NULL.
// @param n New size in bytes.
// @return Resized allocation, or NULL on failure (p stays valid).
...
// @param a Allocator for the copy.
// @param s NUL-terminated string, or NULL.
// @return Copy of s, or NULL if s is NULL or allocation fails.
...
// @return NUL-terminated copy of the first len bytes of s, or NULL on
//   allocation failure.
...
//
// @param utf8 NUL-terminated UTF-8 string, or NULL.
// @param stack_buf Buffer used when the result fits.
// @param stack_cap Capacity of stack_buf in wchar_t.
// @return stack_buf, or a malloc'd buffer the caller frees, or NULL on
...
// Converts UTF-16 to UTF-8. The heap fallback uses `alloc` because the
// result may be handed to callers of rf_getenv_alloc.
//
// @param wide NUL-terminated UTF-16 string, or NULL.
// @param stack_buf Buffer used when the result fits; may be NULL with
//   stack_cap 0 to force a heap result.
// @param stack_cap Capacity of stack_buf in bytes.
// @param alloc Allocator for the heap fallback, or NULL for libc malloc.
// @return stack_buf, or a buffer the caller frees with alloc, or NULL on
...
int rf_path_is_rlocation_valid(const char* path) {
  ...
  // `\` counts as a separator on every platform so that Windows-style
  // traversal is rejected everywhere.
...
// Grows a buffer, doubling its capacity until it fits.
//
// @param buf In/out buffer; unchanged on failure.
// @param cap In/out capacity in bytes; unchanged on failure.
// @param need Required capacity in bytes, including the NUL.
// @param alloc Allocator for realloc.
...
// Copies a string into a growable buffer.
//
// @param buf In/out buffer.
// @param cap In/out capacity in bytes.
// @param alloc Allocator for growth.
...
// Writes the concatenation a + b into a growable buffer.
//
// @param buf In/out buffer; must not alias a or b.
// @param cap In/out capacity in bytes.
// @param alloc Allocator for growth.
...
// @param buf Buffer with capacity greater than new_len.
// @param new_len Length to truncate to.
...
// Body of rf_paths_from operating on caller-owned scratch buffers. On
// success, ownership of each found buffer moves to *out_manifest /
// *out_directory and the scratch pointer is set to NULL; the caller frees
// whatever remains in *mf_buf / *dir_buf.
//
...
// @param is_readable_file Manifest check, or NULL for rf_is_readable_file.
// @param is_directory Directory check, or NULL for rf_is_directory.
// @param predicate_userdata Passed to both predicates.
// @param alloc Allocator for the scratch buffers.
// @param mf_buf In/out manifest scratch buffer.
// @param mf_cap In/out capacity of *mf_buf.
// @param dir_buf In/out directory scratch buffer.
// @param dir_cap In/out capacity of *dir_buf.
// @param out_manifest Receives the manifest path if found.
// @param out_directory Receives the directory if found.
...
static int rf_paths_from_body(
  ...
  if (!dir_valid) {
    // Strip the 9-byte "_manifest" or "/MANIFEST" suffix.
...
// Reads one line, growing the buffer as needed.
//
// @param f Stream to read from; private to the parser.
// @param line_buf In/out buffer; NUL-terminated on success.
// @param line_cap In/out capacity of *line_buf.
// @param a Allocator for growth.
// @return Line length without the trailing \r/\n; -1 at EOF; -2 on I/O
//   error; -3 on allocation failure.
static int rf_read_line(FILE* f, char** line_buf, size_t* line_cap,
                        const rf_allocator* a) {
// The FILE* is private to the parser, so the unlocked getc is safe where
// one is available.
#if defined(_MSC_VER)
...
// Parses a manifest, calling on_entry for each line until a blank line or
// EOF.
//
// @param path UTF-8 manifest path.
// @param on_entry Receives each unescaped key/value; the pointers are only
//   valid during the call. Returning non-zero aborts the parse.
// @param userdata Passed to on_entry.
// @param error_buf Optional buffer for the error message.
// @param error_buf_len Size of error_buf.
// @param a Allocator for the line and unescape scratch buffers.
// @return RF_OK; RF_ERR_IO if the file cannot be opened or read;
//   RF_ERR_FORMAT on a malformed line; RF_ERR_ALLOC on allocation failure;
//   RF_ERR_CALLBACK if on_entry returned non-zero.
...
// Parses a `_repo_mapping` file, calling on_entry for each line until a
// blank line or EOF. A missing file is not an error.
//
// @param path UTF-8 file path.
// @param on_entry Receives each entry's fields; the pointers are only valid
//   during the call. Returning non-zero aborts the parse.
// @param userdata Passed to on_entry.
// @param error_buf Optional buffer for the error message.
// @param error_buf_len Size of error_buf.
// @param a Allocator for the line buffer.
// @return RF_OK, including when the file does not exist; RF_ERR_IO on a
//   read error; RF_ERR_FORMAT on a malformed line; RF_ERR_ALLOC on
//   allocation failure; RF_ERR_CALLBACK if on_entry returned non-zero.
...
// Entries are sorted by (target_apparent, source_repo). `*` sorts before
// every valid repo-name character, so a wildcard entry such as
// ("dep", "ext+*") is the floor of every ("dep", "ext+<anything>") lookup.
typedef struct {
...
// qsort comparator ordering rf_manifest_entry by key bytes, then length.
//
// @param a Pointer to an rf_manifest_entry.
// @param b Pointer to an rf_manifest_entry.
// @return Negative, zero, or positive as a's key sorts before, equal to, or
//   after b's.
...
// Compares two (target_apparent, source_repo) keys lexicographically.
//
// @param ta_a First key's target_apparent bytes.
// @param ta_a_len Length of ta_a.
// @param sr_a First key's source_repo bytes.
// @param sr_a_len Length of sr_a.
// @param ta_b Second key's target_apparent bytes.
// @param ta_b_len Length of ta_b.
// @param sr_b Second key's source_repo bytes.
// @param sr_b_len Length of sr_b.
// @return Negative, zero, or positive as the first key sorts before, equal
//   to, or after the second.
...
// qsort comparator for rf_repo_entry using rf_repo_key_cmp.
//
// @param a Pointer to an rf_repo_entry.
// @param b Pointer to an rf_repo_entry.
// @return Negative, zero, or positive as a sorts before, equal to, or after
//   b.
...
// rf_parse_manifest_into callback that appends an entry to the handle.
//
// @param userdata The rf_runfiles being built.
// @param key Unescaped key bytes.
// @param klen Length of key.
// @param value Unescaped value bytes.
...
// rf_parse_repo_mapping_into callback that appends an entry to the handle.
//
// @param userdata The rf_runfiles being built.
// @param ta Apparent target repository name bytes.
// @param ta_len Length of ta.
// @param src Source repository name bytes; may end in '*'.
// @param src_len Length of src.
// @param tgt Canonical target repository name bytes.
// @param tgt_len Length of tgt.
// @return 0 on success, 1 on allocation failure.
...
// Binary search of the sorted manifest.
//
// @param rf Handle whose manifest to search.
// @param path Bytes to match; not NUL-terminated at prefix_len.
// @param prefix_len Number of leading bytes of path that form the key.
// @return Index of the entry whose key equals the prefix, or -1.
...
// Manifest and directory lookup without validation or repo mapping.
//
// @param rf Handle to look up in.
// @param path NUL-terminated runfiles-root-relative path.
// @param buf Receives the NUL-terminated result when it fits.
// @param buf_cap Size of buf.
// @param needed Receives the result length excluding the NUL, or 0 if not
//   found.
// @return 1 if found and written; 0 if not found; -1 if buf is too small
//   (buf untouched, *needed set).
static int rf_rlocation_unchecked(const rf_runfiles* rf, const char* path,
  ...
  *needed = 0;
  ...
  // A file under a directory that is itself a runfile is not listed;
  // find the longest listed ancestor and append the remainder.
...
// rf_rlocation_unchecked on the concatenation prefix + suffix.
//
// @param rf Handle to look up in.
// @param prefix First part of the path; not NUL-terminated.
// @param prefix_len Length of prefix.
// @param suffix Second part of the path; not NUL-terminated.
// @param suffix_len Length of suffix.
// @param buf Receives the NUL-terminated result when it fits.
// @param buf_cap Size of buf.
// @param needed Receives the result length excluding the NUL.
// @return As rf_rlocation_unchecked, or -2 if the scratch allocation fails.
...
// Frees a handle in any state of construction.
//
// @param rf Handle to free, or NULL for a no-op.
static void rf_free_impl(rf_runfiles* rf) {
  if (!rf) return;
  // Copy the allocator out before rf itself is freed.
...
// Compares an entry against a lookup key.
//
// @param e Entry to compare.
// @param target_apparent Lookup key's apparent repository name bytes.
// @param ta_len Length of target_apparent.
// @param source_repo Lookup key's source repository name bytes.
// @param sr_len Length of source_repo.
// @return Negative, zero, or positive as e sorts before, equal to, or after
//   the key.
...
// Binary search of the sorted repo map.
//
// @param rf Handle whose repo map to search.
// @param target_apparent Lookup key's apparent repository name bytes.
// @param ta_len Length of target_apparent.
// @param source_repo Lookup key's source repository name bytes.
// @param sr_len Length of source_repo.
// @return Index of the first entry greater than the key, or repo_map_count
//   if there is none.
...
// @param r Return value of rf_rlocation_unchecked or
//   rf_rlocation_unchecked_join.
// @return The corresponding public status.
...
rf_rlocation_status rf_rlocation(rf_runfiles* rf, const char* path,
  ...
  *needed = 0;
  ...
  // The floor is the largest entry <= the lookup key. When every entry is
  // greater, the first entry is still checked because it may be a wildcard
  // for this target_apparent.
  ...
  // A wildcard entry matches when its target_apparent equals ours, its
  // source_repo ends in '*', and ours starts with the part before the '*'.
...
// JAVA_RUNFILES duplicates RUNFILES_DIR for the Java launcher.
